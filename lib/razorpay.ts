...
export const razorpayGateway = {
  ...
  async createOrder(input: {
    ...
    try {
      ...
      console.warn("Razorpay createOrder failed, falling back to simulated order:", err);
      return {
        orderId: mockId("order_", input.receipt),
        amountPaise: input.amountPaise,
        status: "created",
  ...
  async debitMandate(input: {
    ...
    try {
      ...
      console.warn("Razorpay debitMandate failed, falling back to simulated payment:", err);
      return {
        paymentId: mockId("pay_", input.orderId),
        orderId: input.orderId,
        status: "captured",
  ...
  async createPaymentLink(input: {
    ...
    try {
      ...
      console.warn("Razorpay createPaymentLink failed, falling back to simulated link:", err);
      const linkId = mockId("plink_", input.referenceId);
      return {
        linkId,
        shortUrl: `https://rzp.io/rzp/${linkId.slice(6, 14)}`,
  ...
  async fetchPaymentLink(linkId: string): Promise<GatewayPaymentLink> {
    if (!isLiveMode() || linkId.startsWith("plink_")) {
      return {
        linkId,
        shortUrl: `https://rzp.io/rzp/${linkId.slice(6, 14)}`,
        status: "created",
    ...
    try {
      ...
      console.warn(`Razorpay fetchPaymentLink(${linkId}) failed:`, err);
      return {
        linkId,
        shortUrl: `https://rzp.io/rzp/${linkId.slice(6, 14)}`,
        status: "created",
  ...
  async fetchPayment(paymentId: string): Promise<GatewayPayment> {
    if (!isLiveMode() || paymentId.startsWith("pay_")) {
      return {
        paymentId,
        status: "captured",
        amountPaise: 100,
    ...
    try {
      ...
      console.warn(`Razorpay fetchPayment(${paymentId}) failed:`, err);
      return {
        paymentId,
        status: "captured",
        amountPaise: 100,
